...
import { BookOpen, Brain, CalendarDays, ChartColumn, ChevronDown, ClipboardList, CreditCard, FileText, GraduationCap, HelpCircle, History, ListChecks, Menu, MessageSquare, NotebookPen, Send, Shield, Trophy, User, Users, X, Star, Presentation } from "lucide-react";
...
export function AppHeader() {
  ...
  return <header className="relative z-[100] bg-blue-600 text-white shadow-sm">
    <div className="mx-auto flex min-h-16 max-w-[1500px] items-center gap-3 px-4 lg:px-6">
      <Link href="/dashboard" className="flex shrink-0 items-center gap-2.5" aria-label="Ir ao painel"><Image src="/logos/logo.png" alt="Logo" width={42} height={42} className="object-contain" /><div className="hidden flex-col 2xl:flex"><span className="text-sm font-bold leading-tight">Conectado em<br />Concursos Públicos SE</span><div className="mt-0.5 flex items-center text-amber-400"><Star className="h-3.5 w-3.5 fill-current stroke-current" /><Star className="h-3.5 w-3.5 fill-current stroke-current" /><Star className="h-3.5 w-3.5 fill-current stroke-current" /><Star className="h-3.5 w-3.5 fill-current stroke-current" /><Star className="h-3.5 w-3.5 fill-current stroke-current" /></div></div></Link>
